...
def login():
    smtps = SMTP_SSL(host="smtp.gmail.com", port=465) # Порт 587 используется для smtp, а 465 для smtps
    smtps.ehlo()
    smtps.login(environ["MY_ADRESS"], environ["MY_PASSWORD"])
...
def send_email(email, smtps, date, filename):
    if not email:
        return
    attach_file = open(f"{getcwd()}\\GENERATED_PDF\\{date}\\{filename}.pdf", 'rb') # Открывает файл в бинарном режиме
